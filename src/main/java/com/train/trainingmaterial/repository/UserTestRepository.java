...
public interface UserTestRepository extends JpaRepository<UserTestEntity, Long> {
  ...
  @Query(
      "select  ut from UserTestEntity ut where  ut.userLessonEntity.id=:userLessonId and"
          + " ut.testEntity.id=:testId")
  List<UserTestEntity> findByUserLessonIdAndTestId(
      @Param(value = "userLessonId") Long userLessonId, @Param(value = "testId") Long testId);
}
