...
public class TagDtoImpl implements TagDto {
    ...
    public UpdateTagResponse updateTag(Long tagId, UpdateTagRequest request) {
        return UpdateTagResponse.builder().success(tagDao.updateTag(tagId,this.toTagEntityFrom(request))).build();
    }
    ...
    private TagDetailResponse TagDetailFrom(TagEntity tagEntity){
        return TagDetailResponse.builder().level(tagEntity.getLevel()).substance(tagEntity.getSubstance()).build();
    }
