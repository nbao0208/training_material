...
public class TagServiceImpl implements TagService {
    ...
    public Response<GetAllTagResponse> getAllTag() {
        return Response.<GetAllTagResponse>builder().id(UUID.randomUUID().toString()).data(tagDto.getAllTag()).build();
    }

    @Override
    public Response<AddTagResponse> addTag(AddTagRequest request) {
        return Response.<AddTagResponse>builder().id(UUID.randomUUID().toString()).data(tagDto.addTag(request)).build();
    }

    @Override
    public Response<UpdateTagResponse> updateTag(Long tagId, UpdateTagRequest request) {
        return Response.<UpdateTagResponse>builder().id(UUID.randomUUID().toString()).data(tagDto.updateTag(tagId,request)).build();
    }

    @Override
    public Response<DeleteTagResponse> deleteTag(Long tagId) {
        return Response.<DeleteTagResponse>builder().id(UUID.randomUUID().toString()).data(tagDto.deleteTag(tagId)).build();
    }
}
