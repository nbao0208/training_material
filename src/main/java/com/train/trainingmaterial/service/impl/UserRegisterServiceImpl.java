...
public class UserRegisterServiceImpl implements UserRegisterService {
    ...
    public Response<RegisterUserResponse> registerUser(RegisterUserRequest request) {
        if (!this.isSameBetween(request.getPassword(),request.getRewritePassword())){
            throw new NotSamePasswordException(request.getRewritePassword() + " is not same as " + request.getPassword());
        }
        return Response.<RegisterUserResponse>builder().id(UUID.randomUUID().toString()).data(userRegisterDto.registerUser(request)).build();
    }
