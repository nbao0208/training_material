...
public class RatingDaoImpl implements RatingDao {
  ...
  @Override
  public String doRating(Long userId, Long lessonId, Long ratingId, List<Long> commentTemplateId, String optionalComment) {
    UserLessonEntity userLesson = findUserCompleteLesson(userId,lessonId);
    List<CommentRatingEntity> commentRatingEntities = this.saveUserLessonRating(userLesson,commentTemplateId);
  ...
  private UserLessonEntity findUserCompleteLesson(Long userId, Long lessonId){
    UserLessonEntity userLesson = userLessonRepository.findByLessonIdAndUserId(lessonId,userId).orElseThrow(()-> new NullValueException("this user is still not learning lesson"));
  ...
  private String responseMessageWithRateLevel(Long ratingId){
    RatingEntity ratingEntity = ratingRepository.findById(ratingId).orElseThrow(()->new NullValueException("404 not found this rating"));
  ...
  private List<CommentRatingEntity> saveUserLessonRating(UserLessonEntity userLesson, List<Long> commentTemplateId){
    List<CommentTemplateEntity> commentTemplateEntities = commentTemplateRepository.findAllCommentTemplateByListOfId(commentTemplateId);
    List<CommentRatingEntity> commentRatingEntities = commentTemplateEntities.stream().map(cm -> CommentRatingEntity.builder().commentTemplateEntity(cm).userLessonEntity(userLesson).build()).toList();
    commentRatingRepository.saveAll(commentRatingEntities);
    return commentRatingEntities;
  }

  private void saveUserOptionalComment(List<CommentRatingEntity> commentRatingEntities, String optionalComment){
    if(optionalComment!=null){
      List<UserOptionalCommentEntity> userOptionalComment = commentRatingEntities.stream().map(ct -> UserOptionalCommentEntity.builder().commentRatingEntity(ct).comment(optionalComment).build()).toList();
      userOptionalCommentRepository.saveAll(userOptionalComment);
    }
  }

  private void saveLessonCommentRating(List<CommentRatingEntity> commentRatingEntities, Long lessonId){
    LessonEntity lessonEntity = lessonRepository.findById(lessonId).orElseThrow(()-> new NullValueException("404 not found this lesson"));
    List<LessonCommentRatingEntity> lessonCommentRating = commentRatingEntities.stream().map(ct->LessonCommentRatingEntity.builder().lessonEntity(lessonEntity).commentRatingEntity(ct).build()).toList();
